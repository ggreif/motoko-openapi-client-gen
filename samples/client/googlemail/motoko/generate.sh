#!/bin/bash
# Regenerate Motoko client from Gmail API OpenAPI spec
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
CONFIG="$REPO_ROOT/bin/configs/motoko-googlemail.yaml"
GENERATED="$SCRIPT_DIR/generated"

cd "$REPO_ROOT"

echo "Generating Motoko client from Gmail API OpenAPI spec..."
...
# Gmail uses dotted operationIds (e.g. gmail.users.messages.send).
# Each focusApis entry is a dotted prefix; operations whose operationId
# starts with that prefix (followed by . or end-of-string) are kept.
...
if [ ${#FOCUS_APIS[@]} -gt 0 ]; then
  echo "focusApis pruning: keeping operations whose operationId matches prefixes: ${FOCUS_APIS[*]}"
  FOCUS_JSON=$(printf '%s\n' "${FOCUS_APIS[@]}" | jq -R . | jq -s .)
  FILTERED_SPEC=$(mktemp -t gmail-focus.XXXXXX).json
...
# --- HTML entity fixups ---
# OpenAPI spec descriptions can leak &lt; / &gt; / &amp; into generated
# Motoko source as string literals.  Patch them before typechecking.
find "$GENERATED/src" -name "*.mo" -exec sed -i.bak \
  -e 's/&lt;/</g' \
  -e 's/&gt;/>/g' \
  -e 's/&amp;/\&/g' \
  {} \; -exec rm -f {}.bak \;

# --- transitive model pruning ---
# The openapi-generator pulls ALL schemas from components/schemas into the
# generated output even when only a small subset of operations is kept.
# Walk the import graph from the kept APIs + Config and delete every Model
# file not reachable via the import graph.
#
# Import formats observed:
#   APIs    → "../Models/<Name>"  (no .mo suffix)
#   Models  → "./<Name>"          (no .mo suffix)
if [ ${#FOCUS_APIS[@]} -gt 0 ]; then
  echo "transitive-prune: computing reachable model closure..."
  NEEDED=$(mktemp)

  # Seed: models directly imported by kept API files and Config
  for seed in "$GENERATED"/src/Apis/*.mo "$GENERATED/src/Config.mo"; do
    [ -f "$seed" ] || continue
    grep -oE '"[.][.]/Models/[^"]*"' "$seed" 2>/dev/null \
      | tr -d '"' | sed 's|.*/||' || true
  done | sort -u > "$NEEDED"

  # Expand: keep adding models reachable from already-known models (fixed point)
  CHANGED=1
  while [ "$CHANGED" = "1" ]; do
    CHANGED=0
    while IFS= read -r model; do
      src="$GENERATED/src/Models/${model}.mo"
      [ -f "$src" ] || continue
      while IFS= read -r dep; do
        if ! grep -qxF "$dep" "$NEEDED"; then
          echo "$dep" >> "$NEEDED"
  ...
  NEEDED_COUNT=$(wc -l < "$NEEDED" | tr -d ' ')
  echo "transitive-prune: keeping $NEEDED_COUNT models: $(tr '\n' ' ' < "$NEEDED")"

  # Delete models not in the reachable set
  REMOVED=0
  ...
  done < <(find "$GENERATED/src/Models" -type f -name '*.mo' 2>/dev/null)
  rm -f "$NEEDED"
  echo "transitive-prune: removed $REMOVED unreachable model files"
fi
