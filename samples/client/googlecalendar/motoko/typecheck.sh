...
TMPBUILD=$(mktemp -d /tmp/googlecalendar-build-test-XXXXXX)
...
[package]
name = "googlecalendar-build-test"
version = "0.0.1"

[dependencies]
googlecalendar-client = "$GENERATED_ABS"

[canisters]
test = "Main.mo"
EOF
cat > "$TMPBUILD/Main.mo" << 'EOF'
import { calendar_events_list } "mo:googlecalendar-client/Apis/EventsApi";
persistent actor {
