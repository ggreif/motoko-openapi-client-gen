...
TMPBUILD=$(mktemp -d /tmp/weatherapi-build-test-XXXXXX)
...
[package]
name = "weatherapi-build-test"
version = "0.0.1"

[dependencies]
weatherapi-client = "$GENERATED_ABS"

[canisters]
test = "Main.mo"
EOF
cat > "$TMPBUILD/Main.mo" << 'EOF'
import { realtimeWeather } "mo:weatherapi-client/Apis/APIsApi";
persistent actor {
